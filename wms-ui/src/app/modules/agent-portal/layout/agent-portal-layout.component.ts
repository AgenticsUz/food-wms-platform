...
@Component({
  selector: 'app-agent-portal-layout',
  standalone: true,
    ...
    <div class="portal-shell" *transloco="let t">
      <header class="portal-header">
        <div class="portal-header-left">
          <i class="pi pi-id-card portal-logo"></i>
          <span class="portal-brand">WMS {{ t('agentPortal.title') }}</span>
        </div>
        <div class="portal-header-right">
          <span class="portal-user">{{ portalService.agent()?.name ?? 'Guest' }}</span>
...
export default class AgentPortalLayoutComponent {
  portalService = inject(AgentPortalService);
}
