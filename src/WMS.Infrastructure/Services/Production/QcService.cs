...
// cheklaydi, yangi yozuvga TenantId'ni StampEntries qo'yadi.
public class QcService : IQcService
{
    private readonly WmsDbContext _db;
    public QcService(WmsDbContext db) => _db = db;

    public async Task<List<QcParameterDto>> GetParametersAsync()
    {
        return await _db.QcParameters
    ...
    public async Task<QcParameterDto> CreateParameterAsync(CreateQcParameterDto dto)
    {
    ...
    public async Task<QcParameterDto> UpdateParameterAsync(Guid id, CreateQcParameterDto dto)
    {
        var p = await _db.QcParameters.FirstOrDefaultAsync(x => x.Id == id)
    ...
    public async Task<List<QcCheckDto>> GetChecksAsync(Guid? transferId, Guid? stageExecutionId)
    {
        var q = _db.QcChecks.AsQueryable();
    ...
    public async Task<QcCheckDto> CreateCheckAsync(Guid userId, CreateQcCheckDto dto)
    {
        ...
        {
            var transferExists = await _db.Transfers.AnyAsync(t => t.Id == dto.TransferId.Value);
            if (!transferExists) throw new NotFoundException("Transfer not found");
        }

        if (dto.StageExecutionId.HasValue)
        {
            // StageExecution endi o'z tenant_id siga ega (D4) — buyurtma orqali JOIN shart emas.
            var stageExecutionExists = await _db.StageExecutions.AnyAsync(se => se.Id == dto.StageExecutionId.Value);
            if (!stageExecutionExists) throw new NotFoundException("Stage execution not found");
        }

        var parameterExists = await _db.QcParameters.AnyAsync(p => p.Id == dto.ParameterId);
        if (!parameterExists) throw new NotFoundException("QC parameter not found");

        var check = new QcCheck
        {
            StageExecutionId = dto.StageExecutionId,
