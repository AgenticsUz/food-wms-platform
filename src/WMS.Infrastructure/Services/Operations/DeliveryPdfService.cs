...
public class DeliveryPdfService : IDeliveryPdfService
{
    ...
    public async Task<byte[]> GenerateWaybillPdfAsync(Guid deliveryId, CancellationToken ct = default)
    {
        ...
        // Brendlash — reports modulining (W1·6) ReportBranding'i; tenant joriy kontekstdan.
        var brand = await ReportBranding.LoadAsync(_db, _files, ct);
    ...
    private static void ComposeHeader(IContainer container, Delivery delivery, ReportBranding brand)
    {
        container.Column(col =>
        {
            col.Item().Row(row =>
            {
                ...
                {
                    // Kalit endi Guid (36 belgi) — int davridagi 14pt sarlavhaga sig'masdi.
                    right.Item().AlignRight().Text("Delivery").Bold().FontSize(14);
                    right.Item().AlignRight().Text($"#{delivery.Id}").FontSize(8).FontColor(TextMuted);
        ...
        container.PaddingVertical(5).Column(col =>
        {
            ...
            col.Item().Table(table =>
            {
                ...
                for (int i = 0; i < stops.Count; i++)
                {
                    ...
                    DataCell(table, stop.SequenceOrder.ToString(System.Globalization.CultureInfo.InvariantCulture), bg);
